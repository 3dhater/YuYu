...
bool Gui::GuiSystem::_init(Gui::GraphicsSystemType type, const char* defaultFontDir, const char* defaultFontFilename)
{
	...
	if( result )
	{
		...
		if(!m_defaultFont)
		{
			m_defaultFont = createFontFromSystem( (const char16_t*)L"Noto Sans", 8, false, false, true, true, true, defaultFontDir, defaultFontFilename );
			
			if(!m_defaultFont)
			{
				m_defaultFont = createFontFromSystem((const char16_t*)L"Courier", 8, false, false, true, true, false );
				if(!m_defaultFont)
				{
