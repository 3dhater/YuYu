...
)
{
	...
	for( size_t i = 0, sz = imgs.size(); i < sz; ++i )
	{
		...
		for( int i2 = 0; i2 < (pbih->biHeight/2); ++i2 )
		{
			memcpy(row, lpBits + (rowsize * i2), rowsize);
			memcpy(lpBits + (rowsize * i2), lpBits + ((pbih->biHeight-1 - i2) * rowsize ) , rowsize);
			memcpy(lpBits + ((pbih->biHeight-1 - i2) * rowsize ), row , rowsize);
		}
