...
yyImage* ImageLoader_DDS(const char* p)
{
	...
	in.read((char*)file_data, file_size);
	...
	auto magic = *reinterpret_cast<const u32*>(file_data);
    if(magic != DDS_MAGIC)
    {
        YY_PRINT_FAILED;
		return 0;
	...
	auto hdr = reinterpret_cast<const DDS_HEADER*>(file_data + sizeof(u32));
