...
D3D11ShaderSpriteBillboard::D3D11ShaderSpriteBillboard()
	:
	m_cb(0)
{
}
...
void D3D11ShaderSpriteBillboard::SetConstants(yyMaterial* material) {
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	g_d3d11->m_d3d11DevCon->Map(m_cb, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	D3D11_BUFFER_DESC d;
	m_cb->GetDesc(&d);
...
bool D3D11ShaderSpriteBillboard::init() {
	const char * text =
		"Texture2D tex2d_1;\n"
		"SamplerState tex2D_sampler_1;\n"
		"struct VSIn{\n"
		...
		"VSOut VSMain(VSIn input){\n"
		"   VSOut output;\n"
		"	output.pos   = mul(WVP, float4(input.position.x, 0.f - input.position.y, 0.0f, 1.f));\n"
		"	output.uv    = input.uv;\n"
